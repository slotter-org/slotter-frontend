...
export function WebSocketProvider({ children }: { children: React.ReactNode }) {
  ...
  const connect = useCallback(() => {
    ...
    const httpBase = import.meta.env.VITE_API_BASE_URL || "http://localhost:8080"
    ...
    ws.onmessage = (evt) => {
      try {
        ...
        console.warn('[WS] Failed to parse inbound msg =>', evt.data)
      }
    }
  ...
  useEffect(() => {
    if (me) {
      ...
      if (wsRef.current) {
        wsRef.current.close(1000, "no user context")
      }
      setConnected(false)
    }
  }, [me, connect, clearReconnectTimer])
